...
public sealed class WorkflowEndpointTests
{
    ...
    /// <summary>A definitive no-effect executor result uses the advertised 503 response.</summary>
    [Fact]
    public async Task PostAsync_ExecutorReportsNoEffect_ReturnsServiceUnavailable()
    {
        await using var factory = new WebApplicationFactory<Program>()
            .WithWebHostBuilder(builder =>
            {
                builder.ConfigureServices(services =>
                {
                    ...
                    services.AddSingleton<IActionExecutor>(
                        new RecordingActionExecutor { Succeeds = false });
                });
            });
        using var client = factory.CreateClient();
        ...
        executionResponse.StatusCode.ShouldBe(HttpStatusCode.ServiceUnavailable);
        var result = await ReadAsync<WorkflowResponse>(executionResponse);
        result.ActionStatus.ShouldBe("blocked_execution_unavailable");
    }
}
