...
/// <summary>Reports the absence of a data-retention schedule for a payment-data vendor.</summary>
internal sealed class MissingRetentionScheduleRule : IRiskRule
{
    public RiskRuleOutcome? Evaluate(RiskRuleContext context) =>
        ...
        !context.Has(EvidenceFactType.DataRetentionScheduleAvailable)
            ? new RiskRuleOutcome(
                RiskLevel.High,
                new RiskReason("RETENTION_SCHEDULE_MISSING", "No data-retention schedule was found."),
                new MissingEvidenceItem("DATA_RETENTION_SCHEDULE", "Data-retention schedule"),
                [])
            : null;
}
